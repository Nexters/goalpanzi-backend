...
public class MissionMemberAcceptanceTest extends AcceptanceTest {
    ...
    @Test
    void 초대코드로_미션에_참여한다() {
        LoginResponse loginHost = 구글_로그인(new GoogleLoginCommand(ID_TOKEN_HOST, EMAIL_HOST)).as(LoginResponse.class);
        MissionDetailResponse mission = 미션_생성(loginHost.accessToken()).as(MissionDetailResponse.class);
        LoginResponse loginMember = 구글_로그인(new GoogleLoginCommand(ID_TOKEN_MEMBER_A, EMAIL_MEMBER_A)).as(LoginResponse.class);
